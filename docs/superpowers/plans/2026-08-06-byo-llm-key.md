...
Review caught defects in the code blocks above. The repository is correct; **this
document is not**. Do not copy a snippet from it without checking the file.

| Where        | What was wrong in the plan                                                                                                                                                                                                                                                                                 | What shipped                                                                                                                                                                                                                                      |
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Task 3       | `import "server-only"`                                                                                                                                                                                                                                                                                     | Omitted — the package is not a dependency here, so it broke the build and the tests that import the module directly. The two-file split already keeps the SDKs out of the browser bundle.                                                         |
| Task 4       | `looksLikeQuota(responseBody)` ran before the `statusCode === undefined` check, so a network failure with quota-ish wording returned `no_credit`/400 instead of `provider_unreachable`/502 — contradicting the plan's own mapping table. The regex also matched any body containing `credit` or `billing`. | Undefined-status check moved first; regex narrowed to real quota signals. The quota check deliberately stays _before_ the 429 branch: OpenAI returns 429 for an empty account as well as for rate limiting, and only the body distinguishes them. |
| Task 6       | The test named "sends the instructions and the prompt to the model" only asserted the model was called once. It passed regardless of what was sent.                                                                                                                                                        | Asserts the system prompt, keyword, description and style all reach the model.                                                                                                                                                                    |
| Task 6       | Silent removal of the old `"Return only valid domain suggestions in JSON format"` instruction and the JSON skeleton.                                                                                                                                                                                       | Removal is correct — `Output.object()` enforces the schema natively per provider and a competing "reply in JSON" instruction degrades that — and now carries a comment saying so.                                                                 |
| Task 7       | `export const generateRequestSchema`                                                                                                                                                                                                                                                                       | **Build-breaking.** Next.js 15 permits only a fixed set of exports from a route module. `npm test` and `tsc --noEmit` were both blind to it; only `npm run build` caught it, three tasks later. Now module-private.                               |
| Tasks 7, 8   | `llmSchema` duplicated byte-for-byte across both routes.                                                                                                                                                                                                                                                   | Extracted to `src/lib/llm-schema.ts`, imported by both.                                                                                                                                                                                           |
| Tasks 7, 8   | The key-leak tests injected `new Error("boom")` and asserted the response lacked `"sk-test"` — trivially true, and blind to the likely regression of returning `error.message` verbatim.                                                                                                                   | Inject an error that contains the key, and mutation-test the assertion.                                                                                                                                                                           |
| Task 10      | No focus management: focus never entered the panel, never returned to the trigger, no `aria-modal`, and Tab escaped behind the scrim.                                                                                                                                                                      | Full focus trap. Implementation also needed an `e.defaultPrevented` guard so Escape inside the Radix `Select` closes only the listbox, not the whole panel.                                                                                       |
| Task 10      | The hydrate effect keyed off `config` identity, so edits abandoned by closing without saving reappeared on reopen.                                                                                                                                                                                         | Re-syncs whenever the panel opens.                                                                                                                                                                                                                |
| Tasks 10, 11 | `useLlmConfig`'s `ready` flag was never consumed, so a returning visitor saw "Connect a model to generate" on first paint.                                                                                                                                                                                 | Both the Generate button and the trigger treat `!ready` as unknown rather than as "no config".                                                                                                                                                    |
| Task 11      | Button label `Generate Domain Names`.                                                                                                                                                                                                                                                                      | Kept the existing `Generate Domain Ideas 🦤`. The dodo voice is the product's identity and was not the plan's to change.                                                                                                                          |

**Process lesson worth keeping:** `npm run build` belongs in every task's verification,
not only the last one. Vitest does not typecheck, and `tsc --noEmit` does not enforce
Next.js's route-module export rules. Each of the three gates caught something the
other two missed.
